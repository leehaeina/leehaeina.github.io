...
function p(arr, selectNum) {
  let result = [];
  if (selectNum === 1) return arr.map((x) => [x]);
  else {
    arr.forEach((v, index) => {
      let fixer = v;
      const rest = arr.filter((x, i) => index !== i);
      const per = p(arr, selectNum - 1);
      const combine = per.map((x) => [fixer, ...x]);
      result.push(...combine);
    });
    return result;
  }
...
function p(arr, count) {
  ...
  if (count === 1) {
    ...
    arr.forEach((x, index) => {
      const fixer = x;
      const rest = arr.filter((v, i) => i !== index);
      const permu = p(rest, count - 1);
      const combine = permu.map((x) => [fixer, ...x]);
      result.push(...combine);
    });
    return result;
  }
...
let set = [];
function set(arr, count, result) {
  if (arr.length === count) {
    return set.push(...result);
  } else {
    arr[count].reduce((_, v, i) => {
      let next = [...result, v];
      dfs(arr, count + 1, next);
    });
  }
}
let setResult = [];
...
new Map([...map.entries()].sort((a, b) => a[1] - b[1]));
//여러배열에서 조합하기
const result = [];
const dfs = (caseArray, level, caseSet) => {
  if (level === caseArray.length) {
